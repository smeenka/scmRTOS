/*****************************************************************************
			 /-o-o-o
	   -o-o-/--o-o-o

Copyright 2012 Anton Smeenk

This file is part of project scmRTOS_CortexM3.

scmRTOS_CortexM3 is free software: you can redistribute it and/or modify it
under the terms of the GNU General Public License as published by the Free
Software Foundation, either version 3 of the License, or (at your option) any
later version.
scmRTOS_CortexM3 is distributed in the hope that it will be useful, but WITHOUT
ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
FOR A PARTICULAR PURPOSE. See the GNU General Public License for more details.

You should have received a copy of the GNU General Public License along with
scmRTOS_CortexM3. If not, see http://www.gnu.org/licenses/.
*****************************************************************************/



#ifndef SERIAL_H
#define SERIAL_H

#include <stdint.h>
#include <scmRTOS.h>

namespace SERIAL {

enum Baudrate {	BAUD_9600 = 9600,
				BAUD_19200 = 19200, 
				BAUD_38400 = 38400, 
				BAUD_57600 = 57600, 
				BAUD_115200 = 115200, 
				BAUD_230400 = 230400} ;

enum Parity {  None = 0, Odd, Even    };


typedef struct{
	uint8_t* data;
	uint16_t size;    // nr of bytes still to send or receive.
	uint16_t index;   // current index  in the buffer
	uint8_t  stopchar; // if zero, count number of bytes will be transmitted or received
					   // if > 0,  on send or receive of this character, the  buffer is ready
					   // and cbBufferReady will be called
					   // note that the stopchar  is also send or stored in the buffer
	uint8_t  channel; // on sending: if zero, no channel byte will be added.
					  // if > 0, this is the first byte  added to the stream
					  // on receiving: if zero channel is disabled. If > zero,
					  // first character in the stream is interpreted as the channel
					  // nr for dispatching
	OS::TEventFlag* event;
}TSerialBuffer;


/* Class: Serial
 *  Base Class for Serial input/output
 *
 */
class Serial  {

public:

	Serial(){};

    /* Function: baud
     *  Set the baud rate of the serial port
     *  
     * Variables:
     *  baudrate - The baudrate of the serial port (default = 115200).
     */
    virtual void setBaudrate(Baudrate baudrate);



    /* Function: format
     *  Set the transmission format used by the Serial port
     *
     * Variables:
     *  bits - The number of bits in a word (5-8; default = 8)
     *  parity - The parity used (SoftwareSerial::None, SoftwareSerial::Odd, SoftwareSerial::Even, SoftwareSerial::Forced1, SoftwareSerial::Forced0; default = SoftwareSerial::None)
     *  stop - The number of stop bits (1 or 2; default = 1)
     */
    /*void format(int bits = 8, Parity parity = Serial::None, int stop_bits = 1); */





/********************************************************
* Blocking interface
********************************************************/

    /* Function: readable
     *  Determine if there is a character available to read from the device
     *  Note that this function should be used only in non interupt driven mode
     *
     * Variables:
     *  returns - 1 if there is a character available to read, else 0
     */
    virtual    int8_t readable();

    /* Function: writeable
     *  Determine if the device is ready with sending previous character
     *  Note that this function should be used only in non interupt driven mode
     *
     * Variables:
     *  returns - 1 if there is space to write a character, else 0
     */
    virtual     int8_t writeable();
    /* Function: write
     *  Write a character
     *  Function blocks until serial port is not busy and than writes the char
     *  Note that this function should be used only in non interrupt driven mode
     * Variables:
     *  c - The character to write to the serial port
     *  returns - 1 on succesfull, 0 on error
     */
    virtual int8_t write(uint8_t c);

    /* Function: writeLine
     *  Write a string, ending with a zero character
     *  Function blocks until the whole string is send
     *  Note that this function should be used only in non interrupt driven mode
     * Variables:
     *  line - The string  to write to the serial port
     *  returns - 1 on succesfull, 0 on error
     */
    virtual int8_t writeLine(char* line);



    /* Function: read
     *  Read a character
	 * 
     * Reads a character from the serial port. This will block until 
     * a character is available. To see if a character is available, 
     * see <readable>
     *  Note that this function should be used only in non interrupt driven mode
     *
     * Variables:
     *  returns - The character read from the serial port, or -1 in case of error
     */
    virtual int16_t read();

/********************************************************
* Non Blocking interface, backed by interrupts
********************************************************/
    /* Function: enableInterruptTX
     *  Enable the interrupt driven TX flow
     *
     * Variables:
     */
	virtual void enableInterruptTx();
	virtual void disableInterruptTx();

	/* Function: enableInterruptRx
     *  Enable the interrupt driven RX flow
     *
     * Variables:
     */
	virtual void enableInterruptRx();
	virtual void disableInterruptRx();



    
   	/*
	 * Function: transmit
	 * transmit a buffer of data via the uart, in non-blocking way, with interrupt help
	 * if the buffer is fully transmitted, call the CB_BufferSend, if registered
	 * Variables:
	 *      txbuf:	 pointer to transmit buffer (owned by caller)
     *  returns - 1 on succesfull, 0 on error, in case the previous transmit was not completed yet
	 */
    virtual int8_t transmit(TSerialBuffer *txbuf);


	/*
	 * Function: transmit
	 * transmit a buffer of data via the uart, in non-blocking way, with interrupt help
	 * if the buffer is fully transmitted, call the CB_BufferSend, if registered
	 * Variables:
	 *      txbuf:	 pointer to transmit buffer (owned by caller)
     *  returns - 1 on succesfull, 0 on error, in case a receive is still in progress
	 */
    virtual int8_t receive(TSerialBuffer *txbuf);
    
    
};

} // namespace


#endif
